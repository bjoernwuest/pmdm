...
/**
 * Creates the login application wired to the given database client.
 *
 ...
 */
export function createLoginApp(dbClient: DBClient) {
    const app = new Elysia().derive(proxyHeadersDerive).decorate("dbClient", dbClient);
    ...
    app.get("/login/client.js", async ({ request }) => {
        ...
        return new Response(bundle.code, {
            headers: {
                ...
                "ETag": bundle.etag,
                "Cache-Control": (!devMode) ? "public, max-age=31536000, immutable" : "no-cache",
            },
        });
    });
